...
def replay_train(mainDQN: dqn.DQN, targetDQN: dqn.DQN, train_batch: list, main_predict: np.array) -> float:
        ...
        for episode in range(MAX_EPISODES):
            ...
            while not done:
                if np.random.rand() < e:
                    ...
                    main_predict = mainDQN.predict(state)
                    action = np.argmax(main_predict)
                ...
                if len(replay_buffer) > BATCH_SIZE:
                    ...
                    loss, _ = replay_train(mainDQN, targetDQN, minibatch, main_predict)
